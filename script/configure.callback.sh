...
docker exec $(docker ps -q -f name=intermw) sed -i "s/bridge\.callback\.url\=.*$/bridge\.callback\.url\=$INTERMW_CALLBACK_URL/g" /etc/inter-iot/intermw/intermw.properties
